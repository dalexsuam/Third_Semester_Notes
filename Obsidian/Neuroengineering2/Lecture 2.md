...
Let’s look again at the diagram we saw in the previous lesson.  
This diagram represents a **feedback control system**, and now we can understand how it can be **populated by the cerebellar microcircuit**.

The cerebellum receives information from two main sources through the **mossy fibers**:

1. The **actual trajectory** — information about what the body or limb is really doing (the real movement happening in space).
2. The **efference copy** — a copy of the motor command that the brain sent to the muscles (what the system _intended_ to do).
    

These two signals — what was planned and what actually occurred — are processed inside the **granular layer** of the cerebellum.  Here, the **granule cells** take these inputs and elaborate them, then send the processed information through their **parallel fibers** to the **Purkinje cells**.

Meanwhile, the system also includes a **feedback controller**.  This controller compares the **actual trajectory** with the **desired trajectory** — in other words, it looks at the **difference between what we wanted to do and what we actually did**.  That difference is the **error signal**.

This **motor command error** is then carried by the **climbing fibers** to the **Purkinje cells**.  
The climbing fibers act as teachers or correctors, telling the cerebellum:

> “This is the error you just made.”

When the climbing fibers fire, they trigger **synaptic plasticity** — specifically, changes in the strength of the connections between the **parallel fibers** and the **Purkinje cells**.  This adjustment allows the cerebellum to **update its internal connections** so that next time, it can better anticipate and minimize that same error.

In this way, the cerebellum functions as an **inverse model**:  it learns the right set of commands needed to achieve the desired movement outcome. Each time an error occurs, the cerebellum fine-tunes the mapping between commands and results, continuously improving accuracy and coordination.

# <span style="color:rgb(223, 109, 109)">Modelling And Simulations</span>

## <span style="color:rgb(239, 179, 1)">Neural Communication: The Action Potential</span>

Before we can model the cerebellum as a whole, we first need to understand the **basic unit** that makes up all of its circuits — the **neuron**. Modeling begins here, because the activity of the cerebellum ultimately emerges from how neurons communicate with one another.

![[Pasted image 20251024224045.png]]

Neurons exchange information through **action potentials**, which are brief, rapid changes in the **membrane voltage**. These changes occur due to the **opening and closing of ion channels** across the cell membrane, controlled by subcellular mechanisms.

If we look at the shape of a typical action potential, we see a very characteristic voltage pattern:

- It starts from the **resting potential**, where the membrane maintains a stable voltage difference between the inside and the outside of the neuron.
- When the neuron receives enough excitatory input, the **membrane potential surpasses a threshold**, unique to each type of neuron.
- Once this threshold is crossed, **sodium channels** open quickly, allowing positive ions to rush in — this causes a **sharp depolarization**.
- Immediately after, **potassium channels** open more slowly, allowing potassium to leave the cell — this drives the **repolarization** and even a brief **hyperpolarization** before returning to the resting potential.
    
This rapid sequence of events — the **action potential** — is the electrical signal that neurons use to transmit information.

Even though neurons can vary greatly in shape, size, and function — such as **granule cells**, **Purkinje cells**, or **Golgi cells** in the cerebellum — they all share this same fundamental mechanism. Each type of neuron has its own characteristic threshold and firing pattern, but they all rely on this **electrochemical process** to generate and propagate action potentials.

Understanding this mechanism is essential, because when we model the cerebellum, we will simulate not only the network structure but also the **dynamics of these action events**. In other words, every complex cerebellar computation begins from these simple but highly precise voltage changes inside individual neurons.

#### **Summarizing**
The brain’s structure and organization already hint at its computational nature. <span style="color:rgb(2, 141, 192)"><b>Information storage</b> depends on the <b>chemical and physical properties</b> of neurons and synapses, while <b>information transmission</b> relies on <b>electrical and electrochemical signaling</b>. In this context, the <b>neuron</b> serves as the brain’s <b>fundamental computational unit</b>, forming the basic element for all neural processing.<br><br></span>
## <span style="color:rgb(239, 179, 1)">Neural Communication: Single Neuron Dynamics</span>

![[Pasted image 20251024225525.png]]

Now, the important step is to move from the **general neuron model**, such as the **Hodgkin–Huxley (HH) model**, to a level where we can represent **different types of neurons** and organize them into **circuits**, forming the computational core of a brain microcircuit.

To achieve this, we need to describe how **different neuronal populations** behave — in other words, their **distinct firing patterns** or **electro-responsive properties**. These are the intrinsic characteristics that differentiate one type of neuron from another.

For example, some neurons display ==**autorhythmic activity**, meaning they generate **spontaneous firing** at a specific frequency **without any external input**. Others are **non-autorhythmic**, meaning they remain **silent** unless they receive stimulation.==

==However, even neurons without autorhythm can exhibit **subthreshold oscillations** — small fluctuations in membrane potential that don’t cause spikes by themselves but make the neuron more likely to fire when a weak input arrives at the right moment.==

This means that each neuron type has a unique **electro-responsive fingerprint**, determined by its membrane dynamics and ion channel composition, which defines how it processes incoming signals even in the absence of external stimulation.

![[Pasted image 20251025103229.png]]

Now, let’s consider the **electro-responsive properties** that depend on **external inputs**. When neurons receive a **constant input current** (not just a brief pulse, but a continuous one), they start firing **action potentials** at a certain frequency.

==If we gradually increase the injected current, we observe that the **firing frequency also increases**. This relationship between the **input current (I)** and the **firing frequency (f)** is called the **current–frequency relationship** (or _I–f curve_). In many neurons, this relationship is roughly **linear** — meaning that stronger inputs produce faster firing rates. However, the **slope** and **shape** of this curve can vary between different neuron types, helping to distinguish their functional classes.==

| ![[Pasted image 20251025103631.png]] | ![[Pasted image 20251025103549.png]] |
| ------------------------------------ | ------------------------------------ |

Additionally, neurons can show a property called **spike-frequency adaptation**. This means that when a constant input current is applied, the neuron initially fires at a high rate, but then the firing frequency gradually decreases over time, settling into a slower rhythm. This adaptation is another key **electro-responsive feature**, revealing how neurons adjust their excitability during sustained stimulation.

![[Pasted image 20251025104532.png]]

==Another important electro-responsive property is the **post-inhibitory rebound burst**. This happens when a neuron with an intrinsic rhythm (an _autorhythm_) is temporarily **inhibited** by a **negative input current**. During inhibition, the neuron’s rhythmic firing stops — it becomes silent. However, when the inhibition ends, instead of simply returning to its normal firing rate, the neuron produces a **burst of rapid spikes**. This short, high-frequency activation immediately after inhibition is what we call a _post-inhibitory rebound_. It reflects the neuron’s ability to “rebound” strongly after being silenced.==

| ![[Pasted image 20251025104629.png]] | ![[Pasted image 20251025104757.png]] |
| ------------------------------------ | ------------------------------------ |



==Another feature is **resonance**. This means that a neuron’s response depends on the **frequency** of the input it receives. Some neurons respond more strongly to inputs at certain frequencies — a behavior closely related to the **subthreshold oscillations** mentioned earlier.==

All these properties — autorhythm, subthreshold oscillations, current–frequency relationship, spike-frequency adaptation, post-inhibitory rebound, and resonance — are examples of **electro-responsive features**. Each neuron population exhibits its own combination of these behaviors.

<span style="font-weight:bold; color:rgb(2, 141, 192)">Let's recap:</span>


| Property                                  | Definition                                                                                                                                                                                                                                |
| ----------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Autorhythm**                            | Some neurons fire **spontaneously**, without any external stimulation. This intrinsic activity comes from their internal electro-responsive mechanisms.                                                                                   |
| **Subthreshold Oscillations (STO)**       | In some neurons, the **membrane potential fluctuates rhythmically** just below the firing threshold. These oscillations make the neuron more likely to spike at specific frequencies.                                                     |
| **Depolarization-Induced Bursting**       | When an external **depolarizing stimulus** (a positive input current) begins, the neuron’s **firing rate suddenly increases**, producing a burst of spikes.                                                                               |
| **Linear Current–Frequency Relationship** | The **firing frequency** of many neurons **increases proportionally** to the strength of the injected current: higher input current leads to higher firing rates.                                                                         |
| **Spike-Frequency Adaptation (SFA)**      | When a neuron receives a **constant input current**, its initial firing rate is high but **gradually decreases over time**, showing adaptation to continuous stimulation.                                                                 |
| **Phase Reset**                           | A neuron’s ongoing rhythmic activity can be **reset** by an external input, realigning its phase or timing in response to the new stimulus.                                                                                               |
| **Post-Inhibitory Rebound Burst**         | After a period of **inhibition** (caused by a negative, hyperpolarizing input), the neuron can produce a **burst of spikes** once the inhibition ends. The intensity and duration of the inhibition determine how strong this rebound is. |
| **Resonance**                             | Some neurons respond **most strongly to inputs at a specific frequency**. This “preferred frequency” produces the maximum firing response, revealing a resonance phenomenon in their electrical behavior.                                 |

Therefore, when building a **computational model** for a specific type of neuron, we must rely on **experimental data** describing its electro-responsive features. The goal is for our model to **accurately reproduce** these characteristic patterns, ensuring it behaves like the real biological neuron it represents.

<span style="font-weight:bold; color:rgb(2, 141, 192)">Let’s explain that more clearly and fluidly:</span>

When we want to model specific neurons — for example, **granule cells**, **Purkinje cells**, or **Golgi cells** — we typically start from a **generic neuron model**, such as the **Hodgkin–Huxley (HH) model**. This general model describes the basic ionic mechanisms that produce an action potential.

However, each neuron type has unique **electro-responsive properties**, so the HH model must be **adapted** or **optimized** to reflect these specific characteristics.

To do this, we rely on **experimental data** that describe the physiological features of each neuron population. For instance, we must determine:

- Does the neuron show **autorhythmic activity** (spontaneous firing)?
- Does it exhibit a **post-inhibitory rebound burst**?
- What is its **resonance frequency**?
- How does it behave under continuous current injection (e.g., **adaptation**, **bursting**, or **linear frequency response**)?
    
Once we know these features, we **tune the parameters** of the generic model — such as ion channel conductances, membrane capacitance, and time constants — to reproduce the observed behaviors of that neuron type.

After building and validating a model for one neuron (for example, the **granule cell**), we can **connect it functionally** to models of other neurons to reconstruct the **microcircuit**.  
In the case of the cerebellum, this means connecting:

- **Granule cells** to **Purkinje cells** (via **parallel fibers**),
- **Purkinje cells** to the **deep cerebellar nuclei (DCN)**, and
- Adding the **climbing fiber input** that modulates Purkinje activity.

By assembling these neuron-specific models, we can simulate how the **cerebellar microcircuit** processes and transforms information — capturing both its structural organization and its computational dynamics.

## <span style="color:rgb(239, 179, 1)">Cerebellar Single Neuron Dynamics</span>

![[Pasted image 20251025110437.png]]

For each cerebellar cell type, we can define characteristics such as whether it has an **autorhythm** (and its frequency), whether it shows **subthreshold oscillations**, **depolarization-induced bursting**, and other properties. These parameters summarize the experimental observations for each neuron population in the cerebellar microcircuit. Based on this information, our next step is to **model each single neuron** according to its specific features.

## <span style="color:rgb(239, 179, 1)">Modelling Single Neurons</span>

==There are different levels of electrical detail when modeling neurons, mainly the **Hodgkin–Huxley (HH)** model and the **Leaky Integrate-and-Fire (LIF)** model.==
![[Pasted image 20251025111553.png]]
![[Pasted image 20251025111741.png]]
In the **HH model**, the **membrane potential (Vm)** is calculated based on the **resting potential ($E_L$)** and the contributions of the different ion channels in the membrane, each represented by a **conductance ($g_c$)** and a **reversal potential ($E_c^{rev}$)**.

This description applies to a **single compartment** — meaning the neuron is treated as one electrical unit. However, to simulate how spikes **propagate along the axon**, the neuron can be divided into **multiple compartments**. Each compartment follows the same HH equation, and **adjacent compartments are linked** through an **internal coupling resistance**, allowing voltage changes to spread through the neuron.

### <span style="color:rgb(161, 40, 226)">Compartmental Model</span>

![[Pasted image 20251025111905.png]]

In a **compartmental model**, each part of the neuron (soma, dendrites, axon, etc.) is modeled separately. For example, a Purkinje cell model might include over **1,600 compartments**, representing its dendritic branches, soma, axon initial segment (AIS), myelinated axon sections, and axon collaterals.

![[Pasted image 20251025111847.png]]

The reason for using such detailed models is that **membrane conductances vary across different parts of the neuron** — dendrites, soma, and axon segments each have distinct ion channel properties. Biologists and physiologists have experimentally measured these variations, so accurate compartmental models must integrate this data.

==In summary, each compartment applies the **HH equations** with **specific conductances** for that region, and all compartments are connected through internal resistances to represent the full neuron realistically.


![[Pasted image 20251025112729.png]]

These are examples of **Hodgkin–Huxley compartmental models**. For instance, **sodium (Na⁺) channels** in the **dendrites** might have a **maximum conductance of 0.016** and a **reversal potential of +60 mV**, based on data from _Raman and Bean (2001)_. Similar data are needed for the **soma**, **axon**, and other compartments, as well as for different ion types — **sodium, potassium, calcium**, etc.

For each compartment, **differential equations** are defined according to these conductances, and **continuity rules** are applied between neighboring compartments to ensure consistent voltage propagation. Using this approach, and with the help of **supercomputers like CINECA**, researchers were able to model a single **Purkinje cell** in high biological detail — a process that required around **two weeks** of computation to optimize all parameters and reproduce the cell’s **electroresponsive properties**.

==However, this level of detail comes with a cost: while it achieves **very high biological realism**, it demands **enormous computational resources**. This makes it impractical for modeling large **microcircuits** involving many interconnected neurons.==

==Therefore, when designing simulations, it’s essential to balance **biological plausibility** with **computational efficiency**. The right choice depends on the **goal of the model** — whether it focuses on the detailed dynamics of a single cell or the collective behavior of larger neural networks — and on the **available computing power**.==

### <span style="color:rgb(161, 40, 226)">Point Neuron Models</span> 

![[Pasted image 20251025113258.png]]

Otherwise, we can use **point neuron models**, where we **collapse all the neuron’s complexity**—its dendrites, axon, and ion channel dynamics—into a **single equation** representing the neuron as a **single point**.

In this simplified approach, we only define basic electrical properties such as **resistance**, **capacitance**, **conductance**, and a **threshold potential**. The subthreshold behavior of the neuron (how the membrane potential changes before a spike) is captured by these passive properties, while the **threshold** determines when a **spike** occurs. 

| ![[Pasted image 20251025113328.png]] | ![[Pasted image 20251025113337.png]] |
| ------------------------------------ | ------------------------------------ |

When the membrane voltage crosses this threshold, the neuron is said to “fire,” and the spike is recorded as an **event in a raster plot** rather than as a full waveform.

These **point neuron models** ignore the morphological and spatial differences within the neuron and instead focus on its **computational behavior**—how it transforms input currents into spike outputs.

==A common example is the **Leaky Integrate-and-Fire (LIF)** model, which only considers **passive membrane properties** (membrane **capacitance** _Cm_ and **resistance** _Rm_). The output is simply a **spike train**, marking the **moments when the voltage threshold is exceeded**.==

| ![[Pasted image 20251025154041.png]] | ![[Pasted image 20251025114455.png]] |
| ------------------------------------ | ------------------------------------ |

The **Leaky Integrate-and-Fire (LIF)** model describes the membrane potential dynamics of a neuron using a simple differential equation. When the **membrane voltage** reaches a defined **threshold**, a **spike event** is registered, and the voltage is reset to a baseline value. In this model, **action potentials** are treated as **discrete events**—that is, only their _occurrence in time_ is represented, without reproducing the actual shape of the spike waveform.

![[Pasted image 20251025114545.png]]
Because of this, comparisons between **experimental electroresponsive properties** of neurons and the model’s output can only be made in terms of **spike frequency** or **spike timing**, not spike morphology.

![[Pasted image 20251025114616.png]]

==As previously mentioned, **Hodgkin-Huxley (HH)** multicompartmental models are highly biologically realistic but computationally demanding, while **LIF models** are computationally efficient but less biologically plausible. This trade-off can be visualized along two axes: **biological plausibility** and **computational cost**. HH models lie at one extreme (high biological realism, high cost), and LIF models at the other (low realism, low cost).

Between these two extremes, there are **multi-dimensional LIF models**, such as the **Izhikevich**, **Adaptive Exponential (AdEx)**, and **Generalized LIF** models. These remain **point neuron models**—they do not model neuron morphology—but they include **additional parameters** to better capture **subthreshold dynamics** and more complex spiking behaviors.

Importantly, **all neuron models depend on parameters**. Some of these are **biological parameters** taken directly from experimental data (for example, conductances or reversal potentials), which ensure **biological plausibility**. Others are **free parameters** that must be **optimized** so that the model reproduces the **electroresponsive features** observed in experiments. This optimization process aligns the model’s behavior with real neuronal responses.

### <span style="color:rgb(161, 40, 226)">Multi-dimensional LIF models: Izhikevich Neuron</span>

![[Pasted image 20251025115023.png]]

The **Izhikevich model** is a **two-dimensional simplification** of the Hodgkin-Huxley framework. Like the LIF model, it represents spikes as **events**, not as continuous waveforms. However, it introduces a **quadratic term** in the voltage equation, which allows for more realistic spike initiation dynamics.

After each spike, the **membrane potential** is reset to a value _c_, while a **recovery variable** is incremented by _d_. This recovery variable represents the **combined effect of slower ionic currents**, summarizing the influence of different ion channels in a single term.

The Izhikevich model can reproduce a wide range of **spiking patterns** observed in cortical and thalamic neurons. Its behavior depends on four main parameters: _(a, b, c, d)_. By adjusting these parameters, one can simulate different types of neuronal responses — for example:

- _(a = 0.02, b = 0.25)_ → **Thalamocortical behavior**, showing linear frequency-current adaptation.
- Other combinations → **Low-threshold spiking (LTS)** neurons or **resonator neurons**, among others.

In summary, the Izhikevich model provides a **good compromise** between biological realism and computational efficiency, allowing the simulation of diverse neuronal behaviors through **parameter tuning** while keeping a **point-neuron structure**.